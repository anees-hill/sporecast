...
class EnsembleCountForecaster(luigi.Task):
    outcome_selection = luigi.Parameter()

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.current_datetime = dt.datetime.now().strftime("%d%m%Y_%H%M")

    def output(self):
        output_file_name1 = f"predicted_counts_{self.outcome_selection}_{self.current_datetime}.csv"
        output_file_name2 = f"xgb_{self.outcome_selection}_{self.current_datetime}.pkl"
        return {
            "output1": luigi.LocalTarget((base_dir/"data"/"processed"/output_file_name1), format=luigi.format.Nop),
            "output2": luigi.LocalTarget((base_dir/"models"/self.outcome_selection/output_file_name2), format=luigi.format.Nop)
        }
    ...
    def run(self):
        ...
        features_file = f"selected_features_{self.outcome_selection}_counts.pkl"
        ...
        peaks_file = f"predicted_peaks_{self.outcome_selection}.pkl"
        ...
        counts_out = pd.concat([counts_out, test_y], axis=1)
          ...
          # Export peaks
        with self.output()["output1"].open("w") as f:
            counts_out.to_csv(f)

        # Export model to model store
        with self.output()["output2"].open("wb") as f:
