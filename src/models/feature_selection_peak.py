...
class FeatureSelectionPeak(luigi.Task):
    outcome_selection = luigi.Parameter()
    use_spore_lags = luigi.BoolParameter()

    def output(self):
        output_file_name = f"selected_features_{self.outcome_selection}_peak.pkl"
    ...
    def run(self):
        ...
        with self.output().open("wb") as f:
            pickle.dump({'selected_features': selected_features_clean,
                        "use_spore_lags": self.use_spore_lags}, f)