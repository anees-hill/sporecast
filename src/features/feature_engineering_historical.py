...
# Select number of lags the models should use
lag_model = 3
...
# Keep an untouched train dataset (with annual outliers kept in), for use as features
...
# Assuming 'date_split' is a datetime.date object
date_split_dt = dt.datetime.combine(date_split, dt.datetime.min.time())

# Convert the wind direction classes to categorical
...
# Calculate thresholds for use in identifying peaks with peak_classify()
...
# Import day of year clusters (note: importing from R script output, need to adapt for python)
...
for name, group in data_groups:
    ...
    processed_group = generate_lagged_features(processed_group, features_to_lag, lag_model)
    ...
    processed_group = merge_non_target_spores(processed_group, name, full_untouched, value_vars, lag_model)
    processed_group = processed_group.iloc[lag_model:,]

    # store the processed group in the dictionary
    split_dataframes[name] = processed_group

# Export
with open(base_dir/"data"/"interim"/"imputed_engineered_historical.pkl", "wb") as f:
